...
# Load cross-platform helpers (sed_inplace)
...
inject_meta() {
  ...
  if ! grep -qi 'name="description"' "$file" 2>/dev/null; then
    if grep -qi "</head>" "$file"; then
      sed_inplace "s|</head>|<meta name=\"description\" content=\"${description}\">\n</head>|i" "$file"
    fi
  fi
