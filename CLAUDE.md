...
| GSC "Excluded by noindex" for taxonomy pages treated as bug | This is **expected behavior**, not a bug. Google discovers `/tags/` and `/categories/` pages through internal links in articles, crawls them, sees the noindex tag, and reports "Excluded by noindex" in GSC. This confirms the system is working correctly: taxonomy pages have noindex + are excluded from sitemap. These GSC reports are informational — no action needed |
| GSC "Crawled - currently not indexed" for `index.xml` (RSS feed) treated as bug | This is **expected behavior**, not a bug. Hugo generates RSS feeds (`index.xml`) because `home = ["HTML", "RSS"]` in hugo.toml. Google discovers the feed URL via `<link rel="alternate" type="application/rss+xml">` in HTML `<head>`. `robots.txt` already has `Disallow: /index.xml` and `Disallow: /*/index.xml`. Google reports the URL as "Crawled - currently not indexed" — this is informational, no action needed |
